...
def are_services_online() -> bool:
    ...
    jellyfin_ok = is_service_online("Jellyfin", BASE_URL)
    ...
    if are_services_online():
        with SessionLocal() as session:
            ...
            for user in users:
                ...
                if not user:
                    logging.warning("Jellyfin user not found.")
                    continue
                
                for show in unique_shows.values():
                    episodes = get_episodes(show["Id"])
                    logging.debug("episodes debug:", episodes)
                    if not episodes:
                        continue
                    if show["Status"] != "Ended":
                        ...
                        try:
                            ...
                            logging.error("Jellyfin Integrity Error:", err)
                            session.rollback()
                            continue

                        except Exception as err:
                            logging.error("Jellyfin refresh error:", err)
                            session.rollback()
                            continue
